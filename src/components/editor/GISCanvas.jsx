...
import {
  ...
  calculateChakbandiGCA, mogaNumberFont, snapChakbandiVertex,
} from "@/lib/gisEngine";
...
const GISCanvas = forwardRef(function GISCanvas(
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    if (vertexDrag.current) {
      ...
      if (obj && obj.points) {
        ...
        // Chakbandi nodes lock onto canal edges + mustateel/muraba killa lines so the
        // chakbandi line never drifts off parcel boundaries when zoomed in.
        const snapped = obj.type === "chakbandi"
          ? snapChakbandiVertex(worldRaw.x, worldRaw.y, others, zoom, snap)
          : computeSnapPosition(worldRaw.x, worldRaw.y, obj.type === "canal" ? "canal" : obj.type === "khal" ? "khal" : obj.type === "road" ? "road" : "chakbandi", others, { ...snap, zoom });
  ...
  const handleMouseDown = useCallback((e) => {
    ...
    if (activeTool === "move") {
      ...
      // Select tool — selection + vertex/node editing. Mustateel & muraba are also
      // draggable here (acts as the Move tool for parcels): a click selects, a drag
      // moves (with grid snap + overlap protection, same as the Move tool).
      const hit = hitTest(worldRaw.x, worldRaw.y, objects);
      ...
      if (hit && hit.mogaGroup && ["mustateel", "muraba"].includes(hit.type)) {
        const groupObjs = objects.filter(o => o.mogaGroup === hit.mogaGroup);
        movingGroupRef.current = {
          groupId: hit.mogaGroup,
          originals: groupObjs.map(o => ({
