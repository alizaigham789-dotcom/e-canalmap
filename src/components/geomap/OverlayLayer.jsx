...
const CANAL_STYLE_COLORS = {
  flat:        { fill: "#1E40AF", stroke: "#172554" },
  ...
  custom:      { fill: "#1E40AF", stroke: "#172554" },
};
function CanalLine({ obj, latlngs, zoom, transform, colorSettings }) {
  ...
  if (fillLatLngs.length === 0) {
    return (
      <Polyline positions={latlngs.map(p => [p.lat, p.lng])} pathOptions={{ color: "#1E40AF", weight: 3, opacity: 0.9, interactive: false }} />
    );
  }
