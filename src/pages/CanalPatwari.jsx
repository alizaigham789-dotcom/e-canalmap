...
import { BookMarked, ChevronRight, ClipboardList, StickyNote, ShieldCheck } from "lucide-react";
import ModuleShell from "@/components/ModuleShell";

const SUB_MODULES = [
  { id: "halqa", label: "My Halqa Registration", labelUrdu: "حلقہ رجسٹریشن", desc: "اپنا حلقہ اور موضع درج کریں — DC کی منظوری کے بعد ٹاسکس خود بخود مقرر" },
...
export default function CanalPatwari() {
  ...
  const handleClick = (s) => {
    ...
    else if (s.id === "halqa") navigate("/canal-patwari/halqa");
  };
  return (
        ...
        {SUB_MODULES.map((s) => (
          ...
          >
            <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-cyan-400 to-teal-300 flex items-center justify-center shadow-md shadow-cyan-400/25 ring-1 ring-white/20 shrink-0">
              {s.id === "mytasks" ? <ClipboardList className="w-5 h-5 text-white" strokeWidth={2} /> : s.id === "notepad" ? <StickyNote className="w-5 h-5 text-white" strokeWidth={2} /> : s.id === "halqa" ? <ShieldCheck className="w-5 h-5 text-white" strokeWidth={2} /> : <BookMarked className="w-5 h-5 text-white" strokeWidth={2} />}
            </div>
