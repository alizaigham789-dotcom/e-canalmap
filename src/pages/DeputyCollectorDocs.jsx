...
import { Landmark, FileBadge2, ChevronRight, ClipboardList } from "lucide-react";
...
export default function DeputyCollectorDocs() {
  ...
  const handleClick = (s) => {
    ...
    else if (s.id === "task-assignment") navigate("/deputy-collector/task-assignment");
  };
  return (
        ...
        {SUB_MODULES.map((s) => (
          ...
          >
            <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-400 to-green-300 flex items-center justify-center shadow-md shadow-emerald-400/25 ring-1 ring-white/20 shrink-0">
              {s.id === "task-assignment" ? <ClipboardList className="w-5 h-5 text-white" strokeWidth={2} /> : <FileBadge2 className="w-5 h-5 text-white" strokeWidth={2} />}
            </div>
