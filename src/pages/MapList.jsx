      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-5">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {filtered.map(map => (
              <div key={map.id} className={`bg-white border rounded-xl p-3 sm:p-3.5 hover:border-blue-300 hover:shadow-md transition-all shadow-sm ${selectedIds.has(map.id) ? "border-blue-500 ring-2 ring-blue-200" : "border-slate-200"}`}>
                <div className="flex items-start gap-2.5 sm:gap-3">
                  <button
                    onClick={() => toggleSelect(map.id)}
                    className={`w-6 h-6 rounded-md border-2 flex items-center justify-center shrink-0 mt-0.5 transition-all ${selectedIds.has(map.id) ? "bg-blue-600 border-blue-600" : "bg-white border-slate-300 hover:border-blue-400"}`}
                    title="Select for bulk print"
                  >
                    {selectedIds.has(map.id) && <Check className="w-3.5 h-3.5 text-white" />}
                  </button>
                  <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-lg bg-gradient-to-br from-blue-50 to-slate-100 flex items-center justify-center shrink-0">
                    <Map className="w-5 h-5 text-blue-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-semibold text-slate-800 line-clamp-2 break-words leading-snug" dir="auto">{map.title || "Untitled Map"}</h3>
                    {(map.village || map.district) && (
                      <div className="flex items-start gap-1 mt-1">
                        <MapPin className="w-3 h-3 text-slate-400 shrink-0 mt-0.5" />
                        <span className="text-xs text-slate-500 line-clamp-1">{[map.village, map.tehsil, map.district].filter(Boolean).join("، ")}</span>
                      </div>
                    )}
                ...
                <div className="flex gap-2 mt-3">
                  <Link to={`/editor?id=${map.id}`} className="flex-1">
                    <button className="w-full py-2 text-xs font-semibold rounded-lg bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 transition-colors">
                      Map Editor
                    </button>
                  </Link>
                  ...
                  <button
                    ...
                    className="px-3 py-2 text-xs font-semibold rounded-lg bg-slate-50 text-slate-600 border border-slate-200 hover:bg-slate-100 transition-colors"
                  ...
                  <button
                    ...
                    className="px-3 py-2 text-xs font-semibold rounded-lg bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 transition-colors"
